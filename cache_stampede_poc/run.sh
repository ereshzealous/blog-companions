...
case "$OUT" in
  captured-output|reports) echo "refusing to write to $OUT: that is the published evidence. Pick another name." >&2; exit 2 ;;
...
say "== cache stampede POC -> $OUT/   ($started)"
...
if [[ $PRE_OK -ne 0 ]]; then
  say "   FAILED — see $OUT/tier1/prerequisites.txt"; say "   need python 3.12+, pytest, and (for Tier 2) a running Docker"; exit 1
fi
...
ASSERTS=$(grep -c '\[ok\]' "$OUT/tier1/run.txt")
TESTS=$(grep -oE '[0-9]+ passed' "$OUT/tier1/pytest.txt" | tail -1)
say "   ${SCEN:-no scenario line} · ${ASSERTS} assertions · pytest: ${TESTS:-did not report}"
...
# tier 2
T2_RUN=0; LIVE_ASSERT=""; LEFTOVER=""
if [[ $TIER1_ONLY -eq 1 ]]; then
  say ""; say "-- Tier 2 · skipped (--tier1)"
  rmdir "$OUT/tier2" 2>/dev/null
else
  say ""; say "-- Tier 2 · live lab on real Redis + PostgreSQL (a few minutes)"
  REPORTS_DIR="$OUT/tier2" CAPTURED_DIR="$OUT/tier1" ./run_all.sh $KEEP >> "$LOG" 2>&1
  T2_RUN=$?
  LIVE_ASSERT=$(grep -oE 'Tier 2 semantic assertions: [0-9]+/[0-9]+ passed' "$OUT/tier2/06c-live-verify.txt" 2>/dev/null | tail -1)
...

{
  ...
  echo "| Tier 1 assertions | 23 | ${ASSERTS:-—} |"
  echo "| Tier 1 tests | 16 passed | ${TESTS:-—} |"
  ...
  if [[ $TIER1_ONLY -eq 0 ]]; then
    ...
    echo "| Tier 2 semantics | 10/10 passed | ${LIVE_ASSERT:-—} |"
  ...
  if [[ -f "$OUT/tier1/summary.txt" ]]; then
    echo "## Tier 1"; echo; echo '```'; sed -n '1,14p' "$OUT/tier1/summary.txt"; echo '```'; echo
  fi
  if [[ -f "$OUT/tier2/06-live-all.txt" ]]; then
    echo "## Tier 2 · live (origin calls counted by PostgreSQL pg_stat_statements)"; echo
    echo '```'; sed -n '/^SUMMARY/,$p' "$OUT/tier2/06-live-all.txt" | sed '/^exit=/d'; echo '```'; echo
    echo "Naive repeated five more times (real arrival timing varies):"; echo
    echo '```'; grep '^L1' "$OUT/tier2/06b-live-naive-repeats.txt" 2>/dev/null; echo '```'; echo
  fi
  echo "## Files"
  echo
  echo '```'
...
# visual report
if python3 report.py "$OUT" > /dev/null 2>&1; then
  say ""; say "   report: $OUT/report.html"
  [[ $OPEN -eq 1 ]] && { command -v open >/dev/null && open "$OUT/report.html"; }
fi

say ""
if [[ $ok -eq 1 ]]; then say "== PASS · ${elapsed}s · summary: $OUT/RESULT.md"; else say "== FAIL · ${elapsed}s · see $OUT/RESULT.md and $LOG"; fi
exit $(( ok == 1 ? 0 : 1 ))
